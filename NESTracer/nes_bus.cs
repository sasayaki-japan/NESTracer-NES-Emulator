...
namespace NESTracer
{
    internal class nes_bus
    {
        ...
        public byte read1(ushort in_address)
        {
            ...
            if (in_address <= 0x1fff) w_out = nes_main.g_nes_6502.read1((ushort)(in_address & 0x07ff));
            else
        ...
        {
            byte w_low = read1(in_address);
            byte w_high = read1((ushort)(in_address + 1));
            return (ushort)(w_low | (w_high << 8));
        }
        //----------------------------------------------------------------
        //write
        //----------------------------------------------------------------
        public void write1(ushort in_address, byte in_val)
        {
			...
			if (in_address <= 0x1fff) nes_main.g_nes_6502.write1((ushort)(in_address & 0x07ff), in_val);
            else
            if (in_address <= 0x3fff) nes_main.g_nes_ppu.write1(in_address, in_val);
            else
            if (in_address == 0x4014) nes_main.g_nes_ppu.dma(in_val);
            else
        ...
        {
            write1(in_address, (byte)in_val);
            write1((ushort)(in_address + 1), (byte)(in_val >> 8));
        }
        //----------------------------------------------------------------
        //bank
        //----------------------------------------------------------------
